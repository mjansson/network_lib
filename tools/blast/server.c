...
blast_server_send_acks(blast_server_t* server) {
	...
	for (isrc = 0, ssize = array_size(server->sources); isrc < ssize; ++isrc) {
		...
		if (time_elapsed(source->last_ack) >
		        0.05f) {  //TODO: Base on round-trip time and back-off rate (back-off reset on recv packet)
...
static void
blast_server_process_handshake(blast_server_t* server, object_t sock,
                               const network_datagram_t datagram, const network_address_t* address) {
	...
	if (handshake->datasize > PACKET_DATA_MAXSIZE) {
		log_warnf(HASH_BLAST, WARNING_BAD_DATA, "Invalid data size %lld from %s", handshake->datasize,
		          addr);
		goto exit;
	}
	...
	log_infof(HASH_BLAST, "Got handshake packet from %s (seq %d, timestamp %lld)", addr,
	          (int)handshake->seq, (tick_t)handshake->timestamp);
	...
	if (source) {
		if (source->writer &&
		        !string_equal_substr(source->writer->name, handshake->name, handshake->namesize)) {
	...
	if (!source->writer) {
		...
		source->last_ack = time_current();
		log_infof(HASH_BLAST, "Prepare transfer of '%.*s' size %lld with token %d from %s",
		          (int)handshake->namesize, handshake->name, handshake->datasize, source->token, addr);
	}
...
static void
blast_server_process_payload(blast_server_t* server, object_t sock,
                             const network_datagram_t datagram, const network_address_t* address) {
	...
	if (!buffer) {
		log_warnf(HASH_BLAST, WARNING_SUSPICIOUS, "Unable to map chunk for payload seq %lld out of range",
		          packet->seq);
		return;
	}
...
static void
blast_server_read(blast_server_t* server, object_t sock) {
...
static void
blast_server_process_network_events(blast_server_t* server) {
	event_block_t* block;
	event_t* event = 0;
