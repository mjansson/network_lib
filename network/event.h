...
\return Network event stream */
NETWORK_API event_stream_t*
network_event_stream(void);

/*! Post network event
\param id Event ID
\param sock Socket associated with event */
NETWORK_API void
network_event_post(network_event_id id, object_t sock);

/*! Get event socket
\param event Network event
\return Socket associated with event, null if no socket or not a network event */
