...
static void
_socket_doflush(socket_t* sock);

static void
_socket_set_blocking_fd(int fd, bool block);

static socket_stream_t*
_socket_stream_allocate(object_t id);
...
static void
_socket_buffer_read(stream_t* stream);
...
static uint64_t
_socket_last_modified(const stream_t* stream);
	...
	if (sockbase->fd != SOCKET_INVALID) {
		if (sock->family != family) {
			FOUNDATION_ASSERT_FAILFORMAT_LOG(HASH_NETWORK,
			                                 "Trying to switch family on existing socket 0x%llx (0x%" PRIfixPTR " : %d) from %u to %u", sock->id,
...
bool
socket_connect(object_t id, const network_address_t* address, unsigned int timeout) {
	bool success = false;
	int err = 0;
	socket_t* sock;
	socket_base_t* sockbase;
...
/*static unsigned int
_socket_buffered_out( const socket_t* sock )
{
	FOUNDATION_ASSERT( sock );
...
_socket_store_address_local(socket_t* sock, int family) {
	...
	if (family == NETWORK_ADDRESSFAMILY_IPV4) {
		...
		FOUNDATION_ASSERT_FAILFORMAT_LOG(HASH_NETWORK,
		                                 "Unable to get local address for socket 0x%llx (0x%" PRIfixPTR
		                                 " : %d): Unsupported address family %u", sock->id, sock, sockbase->fd, family);
		return;
	}
...
static socket_stream_t*
_socket_stream_allocate(object_t id) {
	socket_stream_t* sockstream = memory_allocate(HASH_NETWORK, sizeof(socket_stream_t), 0,
	                                              MEMORY_PERSISTENT | MEMORY_ZERO_INITIALIZED);
	...
	if (id) {
		...
		if (sock) {
			FOUNDATION_ASSERT_MSGFORMAT(sock->stream == sockstream,
			                            "Socket %llx (0x%" PRIfixPTR " : %d): Deallocating stream mismatch, stream is 0x%" PRIfixPTR
			                            ", socket stream is 0x%" PRIfixPTR, id, sock,
...
_socket_eos(stream_t* stream) {
	...
	if (((state != SOCKETSTATE_CONNECTED) || (sockbase->fd == SOCKET_INVALID)) &&
	        !_socket_available_nonblock_read(sock))
		eos = true;
